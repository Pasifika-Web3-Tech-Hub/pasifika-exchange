...
contract PasifikaCrossChainBridge is Ownable {
    ...
    event MessageReceived(bytes32 indexed messageId, uint64 indexed sourceChainSelector, address sender, bytes message);

    event TokenReceived(
        bytes32 indexed messageId, uint64 indexed sourceChainSelector, address sender, address token, uint256 amount
    );

    event ChainAdded(SupportedChains indexed chainIndex, uint64 chainSelector, address routerAddress);
     ...
     */
    function updateChainConfig(SupportedChains chain, uint64 chainSelector, address routerAddress) external onlyOwner {
     ...
     */
    function sendMessage(SupportedChains destinationChain, address receiver, bytes memory message)
        external
        returns (bytes32 messageId)
    {
        ...
        emit MessageSent(messageId, destinationChainSelector, receiver, message, linkToken, fees);
     ...
     * @return messageId The ID of the sent message
     */
    function sendTokens(SupportedChains destinationChain, address receiver, address token, uint256 amount)
        external
        returns (bytes32 messageId)
    {
        uint64 destinationChainSelector = chainSelectors[destinationChain];
        ...
        emit TokenTransferred(messageId, destinationChainSelector, receiver, token, amount, linkToken, fees);
    ...
    function _ccipReceive(Client.Any2EVMMessage memory message) internal {
        ...
        if (message.data.length > 0) {
            emit MessageReceived(messageId, sourceChainSelector, sender, message.data);
        }
    }
